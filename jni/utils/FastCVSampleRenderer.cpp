...
static const uint32_t CORNER_PIXEL_HEIGHT_WIDTH = 5;
...
void drawCorners(uint32_t* corners, uint32_t numCorners) {
	...
	for (uint32_t k = 0; k < numCorners; k++) {
		...
		for (uint32_t i = curcornerx;
				i < CORNER_PIXEL_HEIGHT_WIDTH + curcornerx; i++) {
			for (uint32_t j = curcornery;
					j < CORNER_PIXEL_HEIGHT_WIDTH + curcornery; j++) {
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
void drawColorCorners(uint32_t* corners, uint32_t numCorners,
		uint32_t colorByte1, uint32_t colorByte2) {
	uint32_t curcornerx = 0;
	uint32_t curcornery = 0;
	...
	for (uint32_t k = 0; k < numCorners; k++) {
		...
		for (uint32_t i = curcornerx;
			...
			for (uint32_t j = curcornery;
					j < CORNER_PIXEL_HEIGHT_WIDTH + curcornery; j++) {
				//processing on RGB data
				state.renderBufRGB565[2 * (j * state.renderBufWidth + i)] =
						colorByte1;
				state.renderBufRGB565[2 * (j * state.renderBufWidth + i) + 1] =
						colorByte2;
			}
...
void drawScoreCorners(uint32_t* corners, uint32_t numCorners,
		uint32_t* scores) {
	uint32_t curcornerx = 0;
	uint32_t curcornery = 0;
...
void colorConvertYUV420ToRGB565Renderer(const uint8_t* srcYUV420,
		unsigned int srcWidth, unsigned int srcHeight, uint32_t* dstRGB565) {
...
Java_com_qualcomm_fastcvdemo_utils_FastCVSampleRenderer_cleanup(JNIEnv * env,
		jobject obj) {
	DPRINTF("%s\n", __FUNCTION__);
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
extern "C" JNIEXPORT jboolean JNICALL Java_com_qualcomm_fastcvdemo_utils_FastCVSampleRenderer_drawPixel(
		JNIEnv* env, jobject obj, jint x, jint y, jint color1, jint color2) {
	if (x > 800 || x < 0)
	...
	for (uint32_t i = x; i < CORNER_PIXEL_HEIGHT_WIDTH + x; i++) {
		for (uint32_t j = y; j < CORNER_PIXEL_HEIGHT_WIDTH + y; j++) {
			//processing on RGB data
			state.renderBufRGB565[2 * (j * state.renderBufWidth + i)] = color1;
			state.renderBufRGB565[2 * (j * state.renderBufWidth + i) + 1] =
					color2;
		}
	}
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
extern "C" JNIEXPORT jboolean JNICALL Java_com_qualcomm_fastcvdemo_utils_FastCVSampleRenderer_drawSquare(
		JNIEnv* env, jobject obj, jint xMin, jint xMax, jint yMin, jint yMax, jint color1, jint color2) {
	if (xMax > 800 || xMin < 0 || xMax < xMin)
		return (jboolean)false;

	if (yMax > 500 || yMin < 0 || yMax < yMin)
		return (jboolean)false;

	uint32_t width = CORNER_PIXEL_HEIGHT_WIDTH / 4;

	//Draw two lines along the x axis, at the top and bottom of the square.
	for (uint32_t i = xMin - width; i < xMax + width; i++) {
		//Draw bottom line.
		for (uint32_t j = yMin - width; j < width + yMin; j++) {
			//processing on RGB data
			state.renderBufRGB565[2 * (j * state.renderBufWidth + i)] = color1;
			state.renderBufRGB565[2 * (j * state.renderBufWidth + i) + 1] =
					color2;
		...
		//Draw top line.
		for (uint32_t j = yMax - width; j < width + yMax; j++) {
			//processing on RGB data
			state.renderBufRGB565[2 * (j * state.renderBufWidth + i)] = color1;
			state.renderBufRGB565[2 * (j * state.renderBufWidth + i) + 1] =
					color2;
	...
	//Draw two lines along the y axis, at the left and right of the square.
	for (uint32_t j = yMin - width; j < yMax + width; j++) {
		//Draw left line.
		for (uint32_t i = xMin - width; i < width + xMin; i++) {
			//processing on RGB data
			state.renderBufRGB565[2 * (j * state.renderBufWidth + i)] = color1;
			state.renderBufRGB565[2 * (j * state.renderBufWidth + i) + 1] =
					color2;
		...
		//Draw right line.
		for (uint32_t i = xMax - width; i < width + xMax; i++) {
			//processing on RGB data
			state.renderBufRGB565[2 * (j * state.renderBufWidth + i)] = color1;
			state.renderBufRGB565[2 * (j * state.renderBufWidth + i) + 1] =
					color2;
