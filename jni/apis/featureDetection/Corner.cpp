...
int allocateMemory(int width, int height) {
	...
	if (cornerState.CornerScore == NULL) {
		cornerState.CornerScore = (uint32_t*) fcvMemAlloc(
				MAX_CORNERS_TO_DETECT * sizeof(uint32_t), 16);
	}
...
void updateCorners(uint8_t* data, uint32_t w, uint32_t h) {
	...
	if ((cornerState.scaledImgBuf == NULL) && (cornerState.scalingFactor > 1)) {
		...
		cornerState.scaledImgBuf = (uint8_t*) fcvMemAlloc(
				cornerState.scaledImgWidth * cornerState.scaledImgHeight, 16);
	}

	switch (cornerState.scalingFactor) {
	case 2:
		//scale by factor of 1/2
		if (fcvScaleDownBy2u8(data, w, h, cornerState.scaledImgBuf)) {
	...
	case 4:
		//scale by 1/4
		if (fcvScaleDownBy4u8(data, w, h, cornerState.scaledImgBuf)) {
	...
	case 8:
		...
		dataBufWidth = cornerState.scaledImgWidth;
		dataBufHeight = cornerState.scaledImgHeight;
		fcvScaleDownu8(data, w, h, cornerState.scaledImgBuf,
				cornerState.scaledImgWidth, cornerState.scaledImgHeight);
	...
	if (cornerState.enableGaussian) {
		...
		if (cornerState.blurredImgBuf == NULL) {
			...
			cornerState.blurredImgBuf = (uint8_t*) fcvMemAlloc(
					cornerState.blurredImgWidth * cornerState.blurredImgHeight,
					16);
		}

		// Perform gaussian filtering on scaled data buffer.
		fcvFilterGaussian3x3u8(dataBuf, dataBufWidth, dataBufHeight,
				cornerState.blurredImgBuf, 0);
	...
	case 1:
		fcvCornerFast9u8((uint8_t*) dataBuf, dataBufWidth, dataBufHeight, 0,
				cornerState.cornerThreshold, 7, cornerState.corners,
				MAX_CORNERS_TO_DETECT, &cornerState.numCorners);
		break;

	case 2:
		fcvCornerHarrisu8((uint8_t*) dataBuf, dataBufWidth, dataBufHeight, 0, 7,
				cornerState.corners, MAX_CORNERS_TO_DETECT,
				&cornerState.numCorners, cornerState.cornerThreshold);
		break;

	case 3:
		fcvCornerFast9InMasku8((uint8_t*) dataBuf, dataBufWidth, dataBufHeight,
				0, 7, 10, cornerState.corners, MAX_CORNERS_TO_DETECT,
				&cornerState.numCorners, cornerState.maskArray, dataBufWidth,
				dataBufHeight);
		break;

	case 4:
		fcvCornerHarrisInMasku8((uint8_t*) dataBuf, dataBufWidth, dataBufHeight,
				0, 7, cornerState.corners, MAX_CORNERS_TO_DETECT,
				&cornerState.numCorners, cornerState.cornerThreshold,
				cornerState.maskArray, dataBufWidth, dataBufHeight);
		break;

	case 5:
		fcvCornerFast9Scoreu8((uint8_t*) dataBuf, dataBufWidth, dataBufHeight,
				0, cornerState.cornerThreshold, 7, cornerState.corners,
				cornerState.CornerScore, MAX_CORNERS_TO_DETECT,
				&cornerState.numCorners);
		break;

	case 6:
		fcvCornerFast9InMaskScoreu8((uint8_t*) dataBuf, dataBufWidth,
				dataBufHeight, 0, 7, 10, cornerState.corners,
				cornerState.CornerScore, MAX_CORNERS_TO_DETECT,
				&cornerState.numCorners, cornerState.maskArray, dataBufWidth,
				dataBufHeight);
	case 7:
		fcvCornerFast10u8((uint8_t*) dataBuf, dataBufWidth, dataBufHeight, 0,
				cornerState.cornerThreshold, 7, cornerState.corners,
				MAX_CORNERS_TO_DETECT, &cornerState.numCorners);
		break;
	...
	for (uint32_t i = 0; i < cornerState.numCorners * 2; i++) {
		cornerState.corners[i] = cornerState.corners[i]
				* cornerState.scalingFactor;
	}
}
...
Java_com_qualcomm_fastcvdemo_apis_featureDetection_Corner_cleanup(JNIEnv * env,
		jobject obj) {
	DPRINTF("%s\n", __FUNCTION__);
...
//------------------------------------------------------------------------------
JNIEXPORT void
JNICALL Java_com_qualcomm_fastcvdemo_apis_featureDetection_Corner_updateFastCv(
	...
	if ((int) jimgData & 0xF) {
		...
		if (cornerState.alignedImgBuf == NULL) {
			...
			cornerState.alignedImgBuf = (uint8_t*) fcvMemAlloc(w * h * 3 / 2,
					16);
		}
	...
	uint32_t* rgbData = new uint32_t[w * h];
	fcvColorYUV420toRGB8888u8(pJimgData, w, h, rgbData);

	for (uint32_t i = 0; i < w * h; ++i) {
		uint32_t pxl = rgbData[i];
		uint8_t r = pxl % 256;
		pxl /= 256;
		uint8_t g = pxl % 256;
		pxl /= 256;
		uint8_t b = pxl % 256;
		pxl /= 256;
		uint8_t a = pxl % 256;	//Alpha value.
		pxl /= 256;

		// Zero out pixel if there isn't enough red, or if the ratio of red to blue/green too low.
		if (r < g * 2.4 || //Red is less than 1.5x green
				r < b * 2.4 || //Red is less than 1.5x blue.
				r < 100)       //Red is less than 100 out of 255.
			rgbData[i] = 0xFFFFFFFF; //White output.
		else
			rgbData[i] = 0xFFFF0000; //Red output.
	}

	fcvColorRGBA8888ToYCbCr420PseudoPlanaru8((uint8_t*) rgbData, w, h, 0,
			pJimgData, pJimgData + w * h / 2, 0, 0);
	...
	if (cornerState.disableVF) {
		...
		colorConvertYUV420ToRGB565Renderer(pJimgData, w, h,
	...
	util.setProcessTime(
			(util.getProcessTime() * (29.f / 30.f)) + (float) (timeMs / 30.f));
	...
	if (cornerState.detectionSwitch == 5 || cornerState.detectionSwitch == 6) {
		drawScoreCorners(cornerState.corners, cornerState.numCorners,
...
JNIEXPORT void JNICALL Java_com_qualcomm_fastcvdemo_apis_featureDetection_Corner_setScaling(
		JNIEnv* env, jobject obj, int scaleFactor) {
...
JNIEXPORT void JNICALL Java_com_qualcomm_fastcvdemo_apis_featureDetection_Corner_setFastCornerThreshold(
		JNIEnv* env, jobject obj, int threshold) {
...
JNIEXPORT void JNICALL Java_com_qualcomm_fastcvdemo_apis_featureDetection_Corner_setViewFinderEnable(
		JNIEnv* env, jobject obj, jboolean disableViewFinder)
...
JNIEXPORT void JNICALL Java_com_qualcomm_fastcvdemo_apis_featureDetection_Corner_setOverlayPixelsEnable(
		JNIEnv* env, jobject obj, jboolean enableOverlayPixels) {
...
JNIEXPORT void JNICALL Java_com_qualcomm_fastcvdemo_apis_featureDetection_Corner_setGaussianEnable(
		JNIEnv* env, jobject obj, jboolean enableGaussian) {
...
JNICALL Java_com_qualcomm_fastcvdemo_apis_featureDetection_Corner_setDetection(
		JNIEnv* env, jobject obj, int detectionSwitch) {
...
//My stuff
JNIEXPORT jintArray JNICALL Java_com_qualcomm_fastcvdemo_apis_featureDetection_Corner_getCorners(
		JNIEnv* env, jobject obj) {
	int size = cornerState.numCorners*2; // Each point has an x and y value.
	uint32_t *corners = cornerState.corners;
	...
	for (int i = 0; i < size; i++) {
		fill[i] = corners[i]; // put whatever logic you want to populate the values here.
	}
